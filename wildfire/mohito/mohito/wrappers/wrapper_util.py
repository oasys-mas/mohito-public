...
class shared_wrapper_parr(BaseParallelWrapper):
    ...
    def observe(self) -> Dict[str, torch.Tensor]:
        """
        Return the last observations for all agents.

        Returns:
            Dict[str, torch.Tensor]: the last observations for all agents
        """
        observation = self.env.observe()
        return {
            agent: self.modifiers[agent].modify_obs(obs) if agent in self.modifiers else obs
    ...
    def add_modifiers(self, agents_list: List[str]) -> None:
        ...
        #TODO make this observation be the observation output from the prior wrapper.
