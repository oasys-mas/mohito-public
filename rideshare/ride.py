...
class rideshare():
    ...
    def step(self, step, action_list, openness=True, exo_var=None, num_tasks = None, open_level = 0.3, task_dist = (70, 20, 10)):
        '''
        ...
        if openness:
            ...
            if exo_var == None:
                # sample exo_var from a poisson distribution
                exo_var = np.random.uniform() < open_level

                if exo_var:
                    # call generateTask(next_state)
                    if np.sum(np.vectorize(lambda x: sum(isinstance(i, list) for i in x), otypes=[int])(next_state[3])) > self.num_agents * (self.pool_limit + 1):
                        num_tasks = num_tasks_added = 0 
                        
                    else:
                        task_list = self.generateTask(step = step, num_tasks = None, task_dist = task_dist)
                ...
                # call generateTask() for num_tasks number of tasks
                
                task_list = self.generateTask(step = step, num_tasks=num_tasks)
                num_tasks_added = num_tasks
    ...
    def reset(self, step, driver_locations = None, passengers_list = None, accept_list = None, riding_list = None, num_passengers = 2, epoch = 2):
        '''
